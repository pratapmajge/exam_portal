...
exports.getExams = async (req, res, next) => {
  try {
    ...
    // All authenticated users can see all exams
...
exports.startExam = async (req, res, next) => {
  try {
    ...
    // Initialize or get exam session for student
    let studentEntry = exam.examSessions.find(
    ...
    if (!studentEntry) {
      ...
      exam.examSessions.push(studentEntry);
    }
...
exports.submitExam = async (req, res, next) => {
  try {
    ...
    if (existingResult) {
      logger.warn(`Duplicate submission attempt by student: ${req.user._id}`);
      return next(createError(400, 'You have already submitted this exam'));
    }
...
exports.recordProctoring = async (req, res, next) => {
  try {
    ...
    if (existingResult) {
      ...
      return next(createError(400, 'Exam already submitted'));
    }
